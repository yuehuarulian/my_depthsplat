...
python -m src.main +experiment=arkit_scenes \
...
python -m src.main +experiment=arkit_scenes data_loader.train.batch_size=8 dataset.test_chunk_interval=10 trainer.max_steps=1500 trainer.num_nodes=1 model.encoder.num_scales=2 model.encoder.upsample_factor=4 model.encoder.lowest_feat ure_resolution=1model.encoder.monodepth_vit_type=vits checkpointing.pretrained_monodepth=pretrained/depth-anything/model_s.ckpt output_dir=checkpoints/train


# test
CUDA_VISIBLE_DEVICES=0 python -m src.main +experiment=arkit_scenes \
