...
def TrajStringToMatrix(traj_str):
    ...
    rot = R.from_rotvec([float(tokens[1]), float(tokens[2]), float(tokens[3])]).as_matrix()  # shape (3, 3)
    c2w = torch.eye(4)
    c2w[:3, :3] = torch.tensor(rot, dtype=torch.float32)
    c2w[:3, 3] = torch.tensor([float(tokens[4]), float(tokens[5]), float(tokens[6])], dtype=torch.float32)
